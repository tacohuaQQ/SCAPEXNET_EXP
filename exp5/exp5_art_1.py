...
# =========================
# Significance analysis helpers
# day-level aggregation + paired bootstrap / permutation test
# =========================
def _finalize_mean4_from_stats(stats4):
    outs = [s.finalize() for s in stats4]
    return {
        "rmse_mean4": float(np.nanmean([o["rmse"] for o in outs])),
        "corr_mean4": float(np.nanmean([o["corr"] for o in outs])),
        "r2_mean4":   float(np.nanmean([o["r2"]   for o in outs])),
        "rmse_qTx": outs[0]["rmse"], "corr_qTx": outs[0]["corr"], "r2_qTx": outs[0]["r2"],
        "rmse_qTy": outs[1]["rmse"], "corr_qTy": outs[1]["corr"], "r2_qTy": outs[1]["r2"],
        "rmse_qSx": outs[2]["rmse"], "corr_qSx": outs[2]["corr"], "r2_qSx": outs[2]["r2"],
        "rmse_qSy": outs[3]["rmse"], "corr_qSy": outs[3]["corr"], "r2_qSy": outs[3]["r2"],
    }


def paired_bootstrap_ci(a, b, metric_name="r2", n_boot=5000, seed=42):
    """
    a, b: paired arrays, same length
    返回 diff = (b - a) 的 bootstrap 均值与 95% CI
    对于 R²/corr，b-a > 0 表示 b 更好
    对于 RMSE，建议传入 baseline 和 target 时自己改成 (a-b) 的解释，见下方包装函数
    """
    ...
    n = a.size
    if n < 2:
        return {"n": int(n), "diff_mean": np.nan, "ci_low": np.nan, "ci_high": np.nan}
    ...
    for k in range(n_boot):
        sel = rng.choice(idx0, size=n, replace=True)
        diffs[k] = np.mean(b[sel] - a[sel])
...
def paired_permutation_pvalue(a, b, n_perm=10000, seed=123):
    """
    配对置换检验（sign-flip permutation）。
    原假设：paired difference 的均值 = 0
    返回双侧 p-value
    """
    a = np.asarray(a, dtype=np.float64)
    b = np.asarray(b, dtype=np.float64)
...
def _metric_diff_summary(day_rows, baseline, target, metric_key, higher_is_better=True,
                         n_boot=5000, n_perm=10000, seed=42):
    """
    day_rows: list[dict], 每行至少包含 day_id, method, metric_key
    baseline/target: method names
    metric_key: e.g. "r2_mean4", "rmse_mean4", "r2_qSx", ...
    """
    # 取交集 day
    a_map = {}
    b_map = {}
    ...
    days = sorted(set(a_map.keys()) & set(b_map.keys()))
    if len(days) < 2:
        return {
            "baseline": baseline,
            "target": target,
    ...
    a = np.array([a_map[d] for d in days], dtype=np.float64)
    b = np.array([b_map[d] for d in days], dtype=np.float64)

    if higher_is_better:
        # diff = target - baseline，>0 表示 target 更好
        boot = paired_bootstrap_ci(a, b, metric_name=metric_key, n_boot=n_boot, seed=seed)
        pval = paired_permutation_pvalue(a, b, n_perm=n_perm, seed=seed + 1)
        diff = np.nanmean(b - a)
    else:
        # 对 RMSE，希望 diff > 0 表示 target 更好，所以定义成 baseline - target
        boot = paired_bootstrap_ci(b, a, metric_name=metric_key, n_boot=n_boot, seed=seed)
        pval = paired_permutation_pvalue(b, a, n_perm=n_perm, seed=seed + 1)
        diff = np.nanmean(a - b)
    ...
    return {
        ...
        "metric": metric_key,
        "n_days": len(days),
        "mean_baseline": float(np.nanmean(a)),
        "mean_target": float(np.nanmean(b)),
        "diff": float(diff),
        "ci_low": boot["ci_low"],
...
def evaluate_day_level_metrics_stream(
    models: Dict[str, Optional[nn.Module]],
    model_mlp: nn.Module,
    K_T: float, K_S: float,
    val_loader_full: DataLoader,
    ...
    """
    按 day 聚合每个方法的 day-level metrics。
    返回 rows: list[dict]
    每个 dict 是一个 (day, method) 的统计结果。
    """
    base_methods = ["Zero", "Kgrad", "MLP"]
    methods = base_methods + [k for k in models.keys() if k not in base_methods]

    # day_acc[method][day_id] = [OnlineStats1D(), ... x4]
    day_acc = {m: {} for m in methods}

    def ensure_day(method, day_id):
        if day_id not in day_acc[method]:
            day_acc[method][day_id] = [OnlineStats1D() for _ in range(4)]
        return day_acc[method][day_id]
    ...
    for batch in val_loader_full:
        ...
        def consume_one_method(mname: str, Yp_m: np.ndarray):
            yp_flat = Yp_m[:, m2d]
            stats4 = ensure_day(mname, day_id)
    ...
    for method in methods:
        for day_id, stats4 in day_acc[method].items():
            out = _finalize_mean4_from_stats(stats4)
            row = {"day_id": int(day_id), "method": method}
            row.update(out)
            rows.append(row)

    log(f"✅ Day-level metrics done | days={len(set(r['day_id'] for r in rows))} | time={time.perf_counter()-t0:.1f}s")
    return rows


def run_significance_analysis_from_day_rows(
    day_rows,
    out_dir: str,
    n_coarse: int,
    primary_method: str = "SCAPEX",
    baselines = ("ResCNN", "RCAN_HF", "SCHOPNet_LFDiv"),
    metrics = (
        ("r2_mean4", True),
        ("rmse_mean4", False),
        ("r2_qSx", True),
    ...
    """
    对 primary_method 相对多个 baseline 做显著性检验。
    higher_is_better:
      - R²/corr: True
      - RMSE: False
    """
    ...
    # 保存 day-level csv
    day_csv = os.path.join(out_dir, f"day_level_metrics_n{n_coarse}.csv")
    fieldnames = ["day_id", "method",
                  "rmse_mean4", "corr_mean4", "r2_mean4",
                  "rmse_qTx", "corr_qTx", "r2_qTx",
                  "rmse_qTy", "corr_qTy", "r2_qTy",
                  "rmse_qSx", "corr_qSx", "r2_qSx",
                  "rmse_qSy", "corr_qSy", "r2_qSy"]
    write_rows_csv(day_csv, day_rows, fieldnames=fieldnames)
    log(f"Saved: {day_csv}")
    ...
    sig_csv = os.path.join(out_dir, f"significance_{primary_method}_vs_baselines_n{n_coarse}.csv")
    write_rows_csv(
        sig_csv,
        sig_rows,
        fieldnames=["n", "baseline", "target", "metric", "n_days",
                    "mean_baseline", "mean_target", "diff", "ci_low", "ci_high", "p_perm"]
    )
    log(f"Saved: {sig_csv}")

    # 画两个最常用图：r2_mean4 / rmse_mean4 的 paired day-level difference boxplot
    def _plot_paired_diff(metric_key, higher_is_better, ylabel, fname):
        ...
        for baseline in baselines:
            ...
            if higher_is_better:
                ...
                diff = a - b  # >0 表示 primary 更好（更小 RMSE）

            data.append(diff[np.isfinite(diff)])
            labels.append(f"{primary_method} - {baseline}")
        ...
        ax.set_title(f"Paired day-level improvement | n={n_coarse}")
        ax.tick_params(axis="x", rotation=20)
        savefig_tight(fig, os.path.join(out_dir, fname))

    _plot_paired_diff(
        metric_key="r2_mean4",
        higher_is_better=True,
        ylabel=r"$\Delta$ R² mean(4)  (>0 better)",
        fname=f"significance_boxplot_r2_mean4_n{n_coarse}.png"
    )
    _plot_paired_diff(
        metric_key="rmse_mean4",
        higher_is_better=False,
        ylabel=r"$\Delta$ RMSE mean(4)  (>0 better)",
        fname=f"significance_boxplot_rmse_mean4_n{n_coarse}.png"
    )
...
def run_one_scale(n_coarse: int, var_dir: str, label_dir: str, result_dir: str) -> Dict:
    ...
    # ============ Significance analysis (day-level) ============
    sig_dir = os.path.join(result_dir, "exp5_significance")
    day_rows = evaluate_day_level_metrics_stream(
        models=models,
        model_mlp=model_mlp,
        K_T=K_T, K_S=K_S,
        val_loader_full=val_full_loader,
    ...
    sig_rows = run_significance_analysis_from_day_rows(
        day_rows=day_rows,
        out_dir=sig_dir,
        n_coarse=n_coarse,
        primary_method="SCAPEX",
        baselines=("ResCNN", "RCAN_HF", "SCHOPNet_LFDiv"),
        metrics=(
            ("r2_mean4", True),
            ("rmse_mean4", False),
