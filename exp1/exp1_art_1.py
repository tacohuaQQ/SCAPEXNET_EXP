...
def add_equal_height_colorbar(fig, ax, im, size="4.5%", pad=0.06):
...
def robust_vmax(arr, q=0.995, fallback=1.0):
...
# =========================
def predict_one_method_phys(
    method_name: str,
    models: Dict[str, Optional[nn.Module]],
    model_mlp: nn.Module,
    Xn: torch.Tensor,
    X_phys_np: np.ndarray,
    ocean2d: np.ndarray,
    Y_mean: np.ndarray,
    Y_std: np.ndarray,
    K_T: float,
    ...
    """
    返回单个方法在一个 full-map 样本上的物理空间预测: [4,H,W]
    """
    dTdx = X_phys_np[4]
    dTdy = X_phys_np[5]
    dSdx = X_phys_np[6]
    ...
    if method_name == "Zero":
        yp = np.zeros((4, X_phys_np.shape[1], X_phys_np.shape[2]), dtype=np.float32)

    elif method_name == "Kgrad":
        yp = np.stack([
            -K_T * dTdx, -K_T * dTdy,
            -K_S * dSdx, -K_S * dSdy
        ...
        yp = predict_mlp_full(model_mlp, Xn, Y_mean, Y_std)[0].astype(np.float32, copy=False)

    else:
        model = models[method_name]
        if model is None:
            raise ValueError(f"Model for method={method_name} is None.")

        if (EVAL_TILE and EVAL_TILE > 0) and (method_name in TILED_METHODS):
...
def scapex_forward_debug(model: nn.Module, Xn: torch.Tensor) -> Dict[str, torch.Tensor]:
    """
    只用于实验一可视化。返回 SCAPEX 内部分支和门控。
    输出都在 normalized space。

    为避免 full-map odd size 在 encoder-decoder 中尺寸不匹配，
    先 pad 到 4 的倍数，forward 后再裁剪回原尺寸。
    """
    base = _unwrap_model(model)
    if not isinstance(base, SCAPEXNet):
        raise TypeError("scapex_forward_debug expects a SCAPEXNet model.")

    base.eval()
    B, C, H, W = Xn.shape

    # pad 到 4 的倍数（两次 /2 下采样）
    pad_h = (4 - H % 4) % 4
    pad_w = (4 - W % 4) % 4

    if pad_h > 0 or pad_w > 0:
        Xn_pad = F.pad(Xn, (0, pad_w, 0, pad_h), mode="replicate")
    else:
        Xn_pad = Xn

    with torch.no_grad():
        feat = base.trunk(Xn_pad)

        emb = base.scale_emb(base.scale_id_t.to(feat.device)).view(1, 8, 1, 1)
        gate_bias = emb[:, 0:4]
        pot_bias  = emb[:, 4:8]

        g_logits = base.gate_head(feat) + gate_bias
        g = torch.sigmoid(g_logits)

        alphaT = g[:, 0:1]
        alphaS = torch.clamp(g[:, 1:2], 0.0, base.alpha_s_max)
        betaT  = g[:, 2:3]
        betaS  = g[:, 3:4]
        ...
        # prior expert
        q_prior = base._prior_flux(Xn_pad)

        # structured expert
        pot = base.pot_head(feat) + pot_bias
        q_schop, qd_schop, qr_schop = base._pot_to_flux_parts(pot)

        # residual expert
        rr = base.res_body(feat)
        rr = base.res_conv(rr) + feat
        q_res = base.res_out(rr)

        # final output
        pred = alpha4 * q_prior + beta4 * q_schop + q_res

        # weighted contributions: 更适合解释“最终谁在主导”
        q_prior_w  = alpha4 * q_prior
        q_struct_w = beta4 * q_schop
        q_res_w    = q_res
    ...
    return {
        ...
        "q_prior": crop_hw(q_prior),
        "q_schop": crop_hw(q_schop),
        "q_diss": crop_hw(qd_schop),
        "q_rot": crop_hw(qr_schop),
        "q_res": crop_hw(q_res),

        "q_prior_w": crop_hw(q_prior_w),
        "q_struct_w": crop_hw(q_struct_w),
        "q_res_w": crop_hw(q_res_w),

        "alphaT": crop_hw(alphaT),
        "alphaS": crop_hw(alphaS),
        "betaT": crop_hw(betaT),
        "betaS": crop_hw(betaS),
        "alpha4": crop_hw(alpha4),
        "beta4": crop_hw(beta4),
    }

def pick_representative_sample(val_full_loader: DataLoader):
    """
    从 validation full-map 中选一个“中层 representative sample”。
    这样不会总是挑到最表层，也不至于太深层。
    """
    cache = []
    levs = []
    ...
    for batch in val_full_loader:
        ...
        # 取前 24 个样本里挑一个中位深度附近的
        if len(cache) >= 24:
            break

    if not cache:
        raise RuntimeError("No validation sample available for representative plotting.")

    levs = np.array(levs, dtype=int)
    med_lev = int(np.median(levs))
    idx = int(np.argmin(np.abs(levs - med_lev)))
    return cache[idx]
...
def write_exp1_metric_tables(metrics: Dict, result_dir: str, n_coarse: int):
    """
    为实验一写两个表：
    1) 全方法完整指标表
    2) 按 mean R2 排序的主表
    ...
    rows_full = metrics_to_rows(metrics, n=n_coarse, add_n=True)
    out_full = os.path.join(result_dir, f"exp1_metrics_full_n{n_coarse}.csv")
    write_rows_csv(out_full, rows_full, fieldnames=["n", "method", "component", "rmse", "corr", "r2"])
    ...
    rank_rows = sorted(rank_rows, key=lambda x: x["mean_r2"], reverse=True)
    out_rank = os.path.join(result_dir, f"exp1_ranked_summary_n{n_coarse}.csv")
    with open(out_rank, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(rank_rows[0].keys()))
        w.writeheader()
        for r in rank_rows:
            w.writerow(r)
    log(f"Saved: {out_rank}")


def _robust_sym_lim(arrs, q=0.995, fallback=1.0):
    vals = []
    for a in arrs:
        x = np.asarray(a)
        x = x[np.isfinite(x)]
        if x.size > 0:
            vals.append(np.quantile(np.abs(x), q))
    if not vals:
        return fallback
    v = max(vals)
...
def plot_exp1_feature_maps(
    ...
    代表性 full-map 场图：
    对 qT_x 和 qS_x 分别画
    [True | Kgrad | RCAN_HF | SCHOPNet_LFDiv(or SCHOPNet) | SCAPEX | Error(SCAPEX)]
    """
    os.makedirs(fig_dir, exist_ok=True)

    if selected_methods is None:
        selected_methods = []
        for name in ["Kgrad", "RCAN_HF", "SCHOPNet_LFDiv", "SCHOPNet", "SCAPEX"]:
            if (name == "Kgrad") or (name in models):
                selected_methods.append(name)

    batch = pick_representative_sample(val_full_loader)
    Xn, Y_phys, ocean, file_id, lev, X_phys = batch
    ...
    ocean2d = ocean.numpy()[0]
    Xp = X_phys.numpy()[0]
    file_id0 = int(file_id[0])
    lev0 = int(lev[0])

    preds = {"True": Yt}
    for name in selected_methods:
        preds[name] = predict_one_method_phys(
            method_name=name,
            models=models,
            model_mlp=model_mlp,
            Xn=Xn,
    ...
    for comp_idx, comp_name in comp_plan:
        show_names = ["True"] + selected_methods
        ncols = len(show_names) + 1  # + error(SCAPEX)
        fig, axs = plt.subplots(2, ncols, figsize=(3.0*ncols, 5.8), constrained_layout=True)

        # prediction color scale
        pred_lim = _robust_sym_lim([preds[k][comp_idx] for k in show_names], q=0.995, fallback=1.0)
        err_scapex = preds["SCAPEX"][comp_idx] - Yt[comp_idx] if "SCAPEX" in preds else None
        err_lim = _robust_sym_lim([err_scapex], q=0.995, fallback=1.0) if err_scapex is not None else 1.0

        # 第一行：prediction maps
        for c, name in enumerate(show_names):
            ax = axs[0, c]
            im = ax.imshow(preds[name][comp_idx], cmap="RdBu_r", vmin=-pred_lim, vmax=pred_lim)
            ax.set_title(name)
            ax.axis("off")
        cax = axs[0, -1]
        if err_scapex is not None:
            im = cax.imshow(err_scapex, cmap="RdBu_r", vmin=-err_lim, vmax=err_lim)
            cax.set_title("SCAPEX error")
        cax.axis("off")

        # 第二行：absolute error maps
        for c, name in enumerate(show_names):
            ax = axs[1, c]
            if name == "True":
                bg = np.abs(Yt[comp_idx])
                lim = np.nanquantile(bg[np.isfinite(bg)], 0.995) if np.isfinite(bg).any() else 1.0
                ax.imshow(bg, cmap="magma", vmin=0.0, vmax=lim)
                ax.set_title("|True|")
            else:
                err = np.abs(preds[name][comp_idx] - Yt[comp_idx])
                lim = np.nanquantile(err[np.isfinite(err)], 0.995) if np.isfinite(err).any() else 1.0
                ax.imshow(err, cmap="magma", vmin=0.0, vmax=lim)
                ax.set_title(f"|{name}-True|")
            ax.axis("off")

        # 第二行最后一列：ocean mask / meta
        axs[1, -1].imshow(ocean2d.astype(np.float32), cmap="gray")
        axs[1, -1].set_title(f"mask\nfile={file_id0}, lev={lev0}")
        axs[1, -1].axis("off")

        fig.suptitle(f"Experiment 1 qualitative maps | n={n_coarse} | {comp_name}", fontsize=14)
        savefig_tight(fig, os.path.join(fig_dir, f"exp1_feature_maps_{comp_name}_n{n_coarse}.png"))


def plot_exp1_scapex_internal_maps(
    models: Dict[str, Optional[nn.Module]],
    val_full_loader: DataLoader,
    fig_dir: str,
    n_coarse: int,
    ...
    以及 prior / structured / residual 的强度分布
    """
    if "SCAPEX" not in models or models["SCAPEX"] is None:
        log("SCAPEX model not found, skip internal map plotting.", "warning")
        return
    ...
    dbg = scapex_forward_debug(models["SCAPEX"], Xn)

    ocean2d = ocean[0].cpu().numpy().astype(bool)

    alphaT = dbg["alphaT"][0, 0].float().cpu().numpy()
    alphaS = dbg["alphaS"][0, 0].float().cpu().numpy()
    betaT  = dbg["betaT"][0, 0].float().cpu().numpy()
    betaS  = dbg["betaS"][0, 0].float().cpu().numpy()

    q_prior = dbg["q_prior"][0].float().cpu().numpy()
    q_schop = dbg["q_schop"][0].float().cpu().numpy()
    q_res   = dbg["q_res"][0].float().cpu().numpy()

    mag_prior = np.sqrt(np.sum(q_prior * q_prior, axis=0))
    mag_schop = np.sqrt(np.sum(q_schop * q_schop, axis=0))
    mag_res   = np.sqrt(np.sum(q_res   * q_res,   axis=0))
    ...
    alphaT = apply_mask(alphaT)
    alphaS = apply_mask(alphaS)
    betaT  = apply_mask(betaT)
    betaS  = apply_mask(betaS)

    mag_prior = apply_mask(mag_prior)
    mag_schop = apply_mask(mag_schop)
    mag_res   = apply_mask(mag_res)
    ...
    for i, (arr, ttl) in enumerate(gate_maps):
        ax = axs[0, i]
        im = ax.imshow(arr, cmap="viridis", vmin=0.0, vmax=1.0)
        ax.set_title(ttl, fontsize=18)
        ax.axis("off")
        add_equal_height_colorbar(fig, ax, im, size="4.5%", pad=0.05)

    vmax_prior = robust_vmax(mag_prior, q=0.995, fallback=1.0)
    vmax_schop = robust_vmax(mag_schop, q=0.995, fallback=1.0)
    vmax_res   = robust_vmax(mag_res,   q=0.995, fallback=1.0)

    mag_maps = [
        (mag_prior, r"$\|q_{\mathrm{prior}}\|$",  vmax_prior),
        (mag_schop, r"$\|q_{\mathrm{struct}}\|$", vmax_schop),
        (mag_res,   r"$\|q_{\mathrm{res}}\|$",    vmax_res),
    ]

    for i, (arr, ttl, vmax) in enumerate(mag_maps):
        ax = axs[1, i]
    ...
    axs[1, 3].text(
        0.02, 0.95,
        f"SCAPEX internal maps\nn={n_coarse}\nfile={int(file_id[0])}\nlev={int(lev[0])}",
        va="top", ha="left", fontsize=16, transform=axs[1, 3].transAxes
    )

    savefig_tight(fig, os.path.join(fig_dir, f"exp1_scapex_internal_maps_n{n_coarse}.png"))


def plot_exp1_scapex_dominant_expert_map(
    models: Dict[str, Optional[nn.Module]],
    val_full_loader: DataLoader,
    fig_dir: str,
    n_coarse: int,
    ...
    """
    Dominant expert map:
    每个像素比较 weighted prior / weighted struct / residual 三路贡献的模长，
    显示哪一支主导最终输出。
    """
    if "SCAPEX" not in models or models["SCAPEX"] is None:
        log("SCAPEX model not found, skip dominant expert map plotting.", "warning")
        return
    ...
    q_prior_w  = dbg["q_prior_w"][0].float().cpu().numpy()   # [4,H,W]
    q_struct_w = dbg["q_struct_w"][0].float().cpu().numpy()
    q_res_w    = dbg["q_res_w"][0].float().cpu().numpy()

    mag_prior  = np.sqrt(np.sum(q_prior_w  * q_prior_w,  axis=0))
    mag_struct = np.sqrt(np.sum(q_struct_w * q_struct_w, axis=0))
    mag_res    = np.sqrt(np.sum(q_res_w    * q_res_w,    axis=0))

    stack = np.stack([mag_prior, mag_struct, mag_res], axis=0)  # [3,H,W]
    dominant = np.argmax(stack, axis=0).astype(np.float32)
    dominant[~ocean2d] = np.nan

    total = mag_prior + mag_struct + mag_res + 1e-12
    ratio_prior  = mag_prior  / total
    ratio_struct = mag_struct / total
    ratio_res    = mag_res    / total

    ratio_prior[~ocean2d]  = np.nan
    ratio_struct[~ocean2d] = np.nan
    ratio_res[~ocean2d]    = np.nan

    cmap_dom = ListedColormap(["#4C78A8", "#F58518", "#54A24B"])
    norm_dom = BoundaryNorm([-0.5, 0.5, 1.5, 2.5], cmap_dom.N)

    fig, axs = plt.subplots(2, 2, figsize=(13, 10), constrained_layout=True)

    # dominant expert
    ax = axs[0, 0]
    im = ax.imshow(dominant, cmap=cmap_dom, norm=norm_dom)
    ax.set_title("Dominant expert", fontsize=16)
    ax.axis("off")
    cb = add_equal_height_colorbar(fig, ax, im, size="4.5%", pad=0.05)
    cb.set_ticks([0, 1, 2])
    cb.set_ticklabels(["prior", "struct", "residual"])

    # prior ratio
    ax = axs[0, 1]
    im = ax.imshow(ratio_prior, cmap="Blues", vmin=0.0, vmax=1.0)
    ax.set_title(r"Prior contribution ratio", fontsize=16)
    ax.axis("off")
    add_equal_height_colorbar(fig, ax, im, size="4.5%", pad=0.05)

    # struct ratio
    ax = axs[1, 0]
    im = ax.imshow(ratio_struct, cmap="Oranges", vmin=0.0, vmax=1.0)
    ax.set_title(r"Structured contribution ratio", fontsize=16)
    ax.axis("off")
    add_equal_height_colorbar(fig, ax, im, size="4.5%", pad=0.05)

    # residual ratio
    ax = axs[1, 1]
    im = ax.imshow(ratio_res, cmap="Greens", vmin=0.0, vmax=1.0)
    ax.set_title(r"Residual contribution ratio", fontsize=16)
    ax.axis("off")
    add_equal_height_colorbar(fig, ax, im, size="4.5%", pad=0.05)

    fig.suptitle(
        f"SCAPEX dominant expert analysis | n={n_coarse} | file={int(file_id[0])} | lev={int(lev[0])}",
        fontsize=18
    )

    savefig_tight(fig, os.path.join(fig_dir, f"exp1_scapex_dominant_expert_map_n{n_coarse}.png"))

def plot_exp1_scapex_gate_histogram(
    models: Dict[str, Optional[nn.Module]],
    val_full_loader: DataLoader,
    fig_dir: str,
    n_coarse: int,
    max_samples_total: int = 400000,
):
    """
    统计 validation set 上 SCAPEX 四个 gate 的分布：
    alphaT, alphaS, betaT, betaS
    """
    if "SCAPEX" not in models or models["SCAPEX"] is None:
        log("SCAPEX model not found, skip gate histogram plotting.", "warning")
        return
    ...
    for batch in val_full_loader:
        ...
        m = np.isfinite(aT) & np.isfinite(aS) & np.isfinite(bT) & np.isfinite(bS)
        if np.sum(m) == 0:
            continue
        ...
        remain = max_samples_total - got
        if remain <= 0:
            break

        if aT.size > remain:
            rng = np.random.default_rng(123 + int(got))
            sel = rng.choice(aT.size, size=remain, replace=False)
            aT = aT[sel]
            aS = aS[sel]
            bT = bT[sel]
            bS = bS[sel]

        arr_alphaT.append(aT)
        arr_alphaS.append(aS)
        arr_betaT.append(bT)
        arr_betaS.append(bS)

        got += aT.size
        if got >= max_samples_total:
            break

    if got == 0:
        log("No valid gate samples collected, skip gate histogram.", "warning")
        return

    alphaT_all = np.concatenate(arr_alphaT)
    alphaS_all = np.concatenate(arr_alphaS)
    betaT_all  = np.concatenate(arr_betaT)
    betaS_all  = np.concatenate(arr_betaS)

    fig, axs = plt.subplots(2, 2, figsize=(12, 8), constrained_layout=True)

    gate_data = [
        (alphaT_all, r"$\alpha_T$", axs[0, 0], "tab:blue"),
        (alphaS_all, r"$\alpha_S$", axs[0, 1], "tab:orange"),
        (betaT_all,  r"$\beta_T$",  axs[1, 0], "tab:green"),
        (betaS_all,  r"$\beta_S$",  axs[1, 1], "tab:red"),
    ]

    bins = np.linspace(0.0, 1.0, 41)

    for arr, ttl, ax, color in gate_data:
        ax.hist(arr, bins=bins, density=True, alpha=0.75, color=color, edgecolor="white")
        q25, q50, q75 = np.quantile(arr, [0.25, 0.50, 0.75])

        ax.axvline(q25, linestyle="--", linewidth=1.2, color="k", alpha=0.7)
        ax.axvline(q50, linestyle="-",  linewidth=1.6, color="k", alpha=0.9)
        ax.axvline(q75, linestyle="--", linewidth=1.2, color="k", alpha=0.7)
    ...
    fig.suptitle(f"SCAPEX gate distributions over validation ocean points | n={n_coarse}", fontsize=18)

    savefig_tight(fig, os.path.join(fig_dir, f"exp1_scapex_gate_histogram_n{n_coarse}.png"))

    # 可选：顺手输出一份 summary csv
    rows = [
        {"gate": "alphaT", "mean": float(np.mean(alphaT_all)), "median": float(np.median(alphaT_all)),
         "std": float(np.std(alphaT_all)), "q05": float(np.quantile(alphaT_all, 0.05)),
         "q25": float(np.quantile(alphaT_all, 0.25)), "q75": float(np.quantile(alphaT_all, 0.75)),
         "q95": float(np.quantile(alphaT_all, 0.95)), "n": int(alphaT_all.size)},
        {"gate": "alphaS", "mean": float(np.mean(alphaS_all)), "median": float(np.median(alphaS_all)),
         "std": float(np.std(alphaS_all)), "q05": float(np.quantile(alphaS_all, 0.05)),
         "q25": float(np.quantile(alphaS_all, 0.25)), "q75": float(np.quantile(alphaS_all, 0.75)),
         "q95": float(np.quantile(alphaS_all, 0.95)), "n": int(alphaS_all.size)},
        {"gate": "betaT", "mean": float(np.mean(betaT_all)), "median": float(np.median(betaT_all)),
         "std": float(np.std(betaT_all)), "q05": float(np.quantile(betaT_all, 0.05)),
         "q25": float(np.quantile(betaT_all, 0.25)), "q75": float(np.quantile(betaT_all, 0.75)),
         "q95": float(np.quantile(betaT_all, 0.95)), "n": int(betaT_all.size)},
        {"gate": "betaS", "mean": float(np.mean(betaS_all)), "median": float(np.median(betaS_all)),
         "std": float(np.std(betaS_all)), "q05": float(np.quantile(betaS_all, 0.05)),
         "q25": float(np.quantile(betaS_all, 0.25)), "q75": float(np.quantile(betaS_all, 0.75)),
         "q95": float(np.quantile(betaS_all, 0.95)), "n": int(betaS_all.size)},
    ]

    out_csv = os.path.join(fig_dir, f"exp1_scapex_gate_histogram_summary_n{n_coarse}.csv")
    write_rows_csv(
        out_csv,
        rows,
        fieldnames=["gate", "mean", "median", "std", "q05", "q25", "q75", "q95", "n"]
    )
    log(f"Saved: {out_csv}")

def run_experiment1_package(
    models: Dict[str, Optional[nn.Module]],
    model_mlp: nn.Module,
    K_T: float,
    K_S: float,
    ...
    exp1_dir = os.path.join(result_dir, "experiment1")
    exp1_fig_dir = os.path.join(exp1_dir, "figures")
    os.makedirs(exp1_fig_dir, exist_ok=True)

    # 这一版 plot_exp1_feature_maps 已经会自动画 qT_x 和 qS_x
    plot_exp1_feature_maps(
        models=models,
        model_mlp=model_mlp,
        K_T=K_T,
        K_S=K_S,
        ...
        Y_mean=Y_mean,
        Y_std=Y_std,
        fig_dir=exp1_fig_dir,
        n_coarse=n_coarse,
    )

    plot_exp1_scapex_internal_maps(
        models=models,
        val_full_loader=val_full_loader,
        fig_dir=exp1_fig_dir,
        n_coarse=n_coarse,
    ...
    plot_exp1_scapex_dominant_expert_map(
        models=models,
        val_full_loader=val_full_loader,
        fig_dir=exp1_fig_dir,
        n_coarse=n_coarse,
    ...
    plot_exp1_scapex_gate_histogram(
        models=models,
        val_full_loader=val_full_loader,
        fig_dir=exp1_fig_dir,
        n_coarse=n_coarse,
...
def run_one_scale(n_coarse: int, var_dir: str, label_dir: str, result_dir: str) -> Dict:
    ...
    # =========================
    run_experiment1_package(
        models=models,
        model_mlp=model_mlp,
        K_T=K_T,
        K_S=K_S,
        val_full_loader=vis_full_loader,
        Y_mean=Y_mean,
        Y_std=Y_std,
